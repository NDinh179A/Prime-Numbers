...
//	JButton subtract5Button = new JButton();
	...
	public PrimeGUI()
	{
		...
		titleLabel.setText("Determine Prime Number");
		questionLabel.setText("Please enter an 10-digit phone number.");
		inputNumberLabel.setText("Number:");
		add5Button.setText("   Determine if Prime   ");
//		subtract5Button.setText("Subtract 5");
		answerLabel.setText("");
		nextNumberButton.setText("   New Number   ");
		//*** the 3 buttons are connected to their classes
		add5Button.addActionListener(new determineIfPrime());
...
//		addAndSubtractButtonPanel.add(subtract5Button);
	...
	*/
	...
	class determineIfPrime implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			determinePrimeNumber num = new determinePrimeNumber();
			System.out.println("in determineIfPrime class");
//			doSomething();//*** other methods can be called
			long number = Long.parseLong(inputTextField.getText());
			num.checkIfPrime(number);
			num.getPrimeFactors();
//			number = number + 5;
			String stringNumber;
			ArrayList <Integer> newFactors= new  ArrayList<>();
			newFactors = num.getFactors();
			if(newFactors.size()==1 )
			{
				stringNumber =number + " is a prime number!";
			}
			else
			{
				stringNumber = "Factors are: " + num.getPrimeFactors();
			}
